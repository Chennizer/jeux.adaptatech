...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function applyCategoryButtons(card) {
    ...
    const visibleCategories = state.categories.filter(cat => cat.name !== DEFAULT_CATEGORY_NAME);
    const palette = ['#00796b', '#5c6bc0', '#8e24aa', '#ef6c00', '#00838f', '#2e7d32', '#c2185b'];
    visibleCategories.forEach((cat, index) => {
      ...
      const canRemove = isMember && activeId === 'all-videos';
      if (isMember) {
        chip.classList.add('active');
        if (!canRemove) chip.disabled = true;
      }
      if (cat.id === activeId) {
      ...
      if (!isMember || canRemove) {
        chip.addEventListener('click', (event) => {
          ...
          if (target.urls.includes(src) && activeId === 'all-videos') {
            target.urls = target.urls.filter(item => item !== src);
            setCategoryState(nextState);
            applyCategoryButtons(card);
            return;
          }
          if (!target.urls.includes(src)) target.urls.push(src);
          setCategoryState(nextState);
          applyCategoryButtons(card);
        });
      }
