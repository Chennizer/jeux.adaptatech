(function() {
  ...
  const launchBuilderBtn = document.getElementById('start-game-button');
  ...
  function setSelection(image, thumbEl) {
    ...
    selectionStatus.textContent = image ? `Selected: ${image.name}` : 'Selected: none';
  }
  ...
  function setActiveStep(index) {
    ...
    activeStatus.textContent = `Active: step ${index + 1}`;
    renderActiveSteps();
  }
  ...
  function markCompleted(index) {
    ...
    updateStepStateClasses(stepsContainer.querySelector(`[data-index="${index}"]`), index);
    activeStepIndex = null;
    activeStatus.textContent = `Completed: step ${index + 1}`;
    renderActiveSteps();
  }
  ...
  function toggleActiveMode() {
    ...
    if (!isActiveMode) {
      activeStatus.textContent = 'Active Mode: off';
    ...
    toggleActiveBtn.textContent = 'Exit Active Mode';
      activeStatus.textContent = 'Active Mode: on';
    renderActiveSteps();
  ...
  function resetProgress() {
    ...
    activeStatus.textContent = isActiveMode ? 'Active Mode: reset' : 'Active Mode: off';
  }
