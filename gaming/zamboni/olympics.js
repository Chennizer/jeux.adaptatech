...
function spawnSnowBurst(){
  ...
  for(let i=0;i<220;i++){
    const angle = (-Math.PI / 3.6) + Math.random() * (Math.PI / 2.1);
    const speed = 3.8 + Math.random() * 5.4;
...
function updateSnowParticles(now){
  if(!snowStage.lastTime) snowStage.lastTime = now;
  const dt = Math.min(48, now - snowStage.lastTime);
  snowStage.lastTime = now;

  const cannon = getCannonBounds();
  const startX = cannon.x + cannon.w * 0.7;
  const startY = cannon.y + cannon.h * 0.2;
  snowStage.emitters = snowStage.emitters.filter(emitter => now - emitter.start < emitter.duration);
  snowStage.emitters.forEach(() => {
    const spawnCount = Math.floor(18 * (dt / 16));
...
function drawSnowStream(now){
  const cannon = getCannonBounds();
  const baseX = cannon.x + cannon.w * 0.7;
  const baseY = cannon.y + cannon.h * 0.2;
  const active = snowStage.emitters.filter(emitter => now - emitter.start < emitter.duration);
  if(!active.length) return;

  ctx.save();
  ctx.globalCompositeOperation = 'screen';
  active.forEach((emitter, index) => {
    const progress = (now - emitter.start) / emitter.duration;
    const spread = 0.18 + progress * 0.08;
    const length = 320 + progress * 180;
    const sway = Math.sin(now * 0.002 + index) * 10;
    const grad = ctx.createLinearGradient(baseX, baseY, baseX + length, baseY - length * 0.4);
    grad.addColorStop(0, 'rgba(255,255,255,0.6)');
    grad.addColorStop(0.5, 'rgba(220,245,255,0.35)');
    grad.addColorStop(1, 'rgba(255,255,255,0)');
    ctx.fillStyle = grad;
    ctx.beginPath();
    ctx.moveTo(baseX, baseY);
    ctx.lineTo(baseX + length, baseY - length * (0.4 + spread) + sway);
    ctx.lineTo(baseX + length, baseY - length * (0.1 - spread) + sway);
    ctx.closePath();
    ctx.fill();
  });
  ctx.restore();
