export function createZamboniGame(p) {
  ...
  const getUiScale = () => Math.max(0.55, Math.min(1.6, Math.min(p.width / BASE_W, p.height / BASE_H)));
  ...
  const cleanSegment = (row, fromCol, toCol) => {
  ...
  const roundRect = (ctxRef, x, y, w, h, r) => {
  ...
  const drawZamboni = (inner, cellW, cellH, s) => {
    ...
    if (zamboniImg.complete && zamboniImg.naturalWidth > 0) {
      ...
      context.drawImage(zamboniImg, dx, 0, drawW, drawH);
    } else {
      context.fillStyle = '#ffc107';
      const dx = flipX === -1 ? -maxW : 0;
      roundRect(context, dx, 0, maxW, maxH * 0.7, 12 * s);
      context.fill();
    }
  ...
  const drawShine = (inner, s) => {
    ...
    const context = ctx();
    context.save();
    roundRect(context, inner.x, inner.y, inner.w, inner.h, 18 * s);
    context.clip();

    const grad = context.createLinearGradient(inner.x, inner.y, inner.x + inner.w, inner.y + inner.h);
    ...
    if (state.finished) {
      ...
      const sweep = context.createLinearGradient(sweepX, inner.y, sweepX + 160 * s, inner.y);
  ...
  const drawSnow = bounds => {
    const context = ctx();
    context.save();
    context.fillStyle = 'rgba(255,255,255,0.8)';
    sparkle.flakes.forEach(flake => {
      context.beginPath();
      context.arc(bounds.x + flake.x * bounds.w, bounds.y + flake.y * bounds.h, flake.size, 0, Math.PI * 2);
      context.fill();
    });
    context.restore();
  };

  const drawSparkles = (bounds, now, s) => {
    ...
    sparkle.orbs.forEach(orb => {
      ...
      const grad = context.createRadialGradient(ox, oy, 0, ox, oy, orb.size * pulse);
      grad.addColorStop(0, `hsla(${orb.hue},80%,65%,0.5)`);
      grad.addColorStop(1, 'rgba(0,0,0,0)');
      context.fillStyle = grad;
      context.beginPath();
      context.arc(ox, oy, orb.size * pulse, 0, Math.PI * 2);
      context.fill();
    });
    ...
    sparkle.stars.forEach(star => {
      ...
      context.strokeStyle = `rgba(255,255,255,${0.35 * twinkle})`;
      context.lineWidth = 1.2 * s;
      context.beginPath();
      context.moveTo(sx - star.r, sy);
      context.lineTo(sx + star.r, sy);
      context.moveTo(sx, sy - star.r);
      context.lineTo(sx, sy + star.r);
      context.stroke();
    });
  ...
  const drawVictoryLap = (bounds, now) => {
    const { x, y, w, h } = bounds;
  ...
  const drawVictoryConfetti = (now, s) => {
    const vw = p.width;
    const vh = p.height;
    const count = 80;
    const size = 6 * s;
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      context.fillStyle = `hsla(${(i * 27) % 360},80%,60%,0.85)`;
  ...
  const drawRink = (bounds, s) => {
    ...
    const context = ctx();
    context.save();
    roundRect(context, x, y, w, h, rOuter);
    context.fillStyle = '#0e2236';
    context.fill();
    roundRect(context, x + 6 * s, y + 6 * s, w - 12 * s, h - 12 * s, rMid);
    context.fillStyle = '#f8fbff';
    context.fill();
    context.restore();

    const iceGrad = context.createLinearGradient(inner.x, inner.y, inner.x, inner.y + inner.h);
    iceGrad.addColorStop(0, '#eefbff');
    iceGrad.addColorStop(1, '#d7f1ff');
    context.fillStyle = iceGrad;
    roundRect(context, inner.x, inner.y, inner.w, inner.h, rInner);
    context.fill();
    ...
    for (let r = 0; r < grid.rows; r += 1) {
      for (let c = 0; c < grid.cols; c += 1) {
        ...
        context.fillStyle = (c + r) % 2 === 0 ? 'rgba(195,224,240,0.42)' : 'rgba(186,217,236,0.38)';
        context.fillRect(cx, cy, cellW + 0.6 * s, cellH + 0.6 * s);
      }
    }
    context.restore();
    ...
    context.lineWidth = Math.max(2 * s, inner.w * 0.004);
    context.strokeStyle = 'rgba(229,57,53,0.9)';
    context.beginPath();
    context.moveTo(centerX, inner.y);
    context.lineTo(centerX, inner.y + inner.h);
    context.stroke();

    context.strokeStyle = 'rgba(30,136,229,0.85)';
    const blueOffset = inner.w * 0.22;
    context.beginPath();
    context.moveTo(centerX - blueOffset, inner.y);
    context.lineTo(centerX - blueOffset, inner.y + inner.h);
    context.moveTo(centerX + blueOffset, inner.y);
    context.lineTo(centerX + blueOffset, inner.y + inner.h);
    context.stroke();

    context.strokeStyle = 'rgba(229,57,53,0.85)';
    context.lineWidth = Math.max(2 * s, inner.w * 0.005);
    context.beginPath();
    context.arc(centerX, centerY, inner.w * 0.09, 0, Math.PI * 2);
    context.stroke();
  ...
  const draw = () => {
    ...
    if (state.finished && now - state.celebrationTime > 4000) {
      complete = true;
    }
  };
