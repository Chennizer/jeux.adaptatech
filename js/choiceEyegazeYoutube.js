document.addEventListener('DOMContentLoaded', () => {
  ...
  function resetToChoicesScreen() {
    ...
    // Ensure the interface remains in fullscreen so selections continue
    // to work without the user needing to manually re-enter fullscreen.
    ...
    if (isYouTubeUrl(videoUrl)) {
      ...
      if (!youtubePlayer) {
        youtubePlayer = new YT.Player('youtube-player', {
          ...
          videoId: id,
          playerVars: { rel: 0, modestbranding: 1, controls: 0, start: resumeTime },
          events: {
            onReady: () => { try { youtubePlayer.playVideo(); } catch {} },
            onStateChange
          }
        });
      } else {
        youtubePlayer.loadVideoById({ videoId: id, startSeconds: resumeTime });
