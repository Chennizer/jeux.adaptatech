...
function spawnFirework(){
  ...
  const hues = [5, 45, 195, 140, 320];
  const hue = hues[openingStage.ringPresses % hues.length];
  const count = 220;
  for(let i=0;i<count;i++){
    ...
    openingStage.fireworks.push({
      ...
      hue: (hue + Math.random() * 40 - 20 + i * 0.15) % 360,
...
function updateFireworks(dt){
  ...
  openingStage.fireworks.forEach(p => {
    ...
    p.vy = p.vy * Math.pow(p.drag, dt * 60) + p.gravity * (dt * 60);
    p.px = p.x;
    p.py = p.y;
    p.x += p.vx * (dt * 60);
    p.y += p.vy * (dt * 60);
...
function drawOpeningStage(now){
  const vw = window.innerWidth;
  const vh = window.innerHeight;
  const dt = openingStage.lastTime ? Math.min(0.05, (now - openingStage.lastTime) / 1000) : 0.016;
  openingStage.lastTime = now;
  ...
  openingStage.fireworks.forEach(p => {
    ...
    const alpha = Math.max(0, (1 - t) * (0.7 - 0.4 * t));
    ctx.strokeStyle = `hsla(${p.hue}, 95%, 70%, ${alpha})`;
    ctx.lineWidth = Math.max(1, p.size * (1.6 - t));
    ctx.beginPath();
    ctx.moveTo(p.px, p.py);
    ctx.lineTo(p.x, p.y);
    ctx.stroke();

    ctx.fillStyle = `hsla(${(p.hue + 20) % 360}, 95%, 80%, ${alpha * 0.8})`;
    ctx.beginPath();
...
function drawOlympicRings(now){
  ...
  rings.forEach((ring, index) => {
    ...
    ctx.lineWidth = ringSize * (0.14 - t * 0.05);
    ctx.globalAlpha = t;
    ctx.beginPath();
...
function spawnSnowBurst(){
  ...
  for(let i=0;i<160;i++){
    const angle = (-Math.PI / 3) + Math.random() * (Math.PI / 2.2);
...
function drawSnowStage(){
  ...
  grad.addColorStop(0, '#5fb1ff');
  grad.addColorStop(1, '#d7f2ff');
  ctx.fillStyle = grad;
  ctx.fillRect(0,0,vw,vh);
...
function buildStars(){
  const vw = window.innerWidth;
  const vh = window.innerHeight;
  const count = Math.floor((vw * vh) / 18000);
...
function drawStars(now){
  ...
  openingStage.stars.forEach(star => {
    const twinkle = 0.5 + Math.sin(now * 0.001 + star.twinkle) * 0.3;
...
function drawProjectorLights(){
  ...
  const vh = window.innerHeight;
  const baseY = vh * 0.86;
  const beamHeight = vh * 0.28;
  const beamWidth = vw * 0.18;
  const centers = [vw * 0.25, vw * 0.5, vw * 0.75];
  ctx.save();
  centers.forEach((cx, index) => {
    const grad = ctx.createLinearGradient(cx, baseY, cx, baseY - beamHeight);
    const hue = 180 + index * 50;
    grad.addColorStop(0, `hsla(${hue}, 80%, 60%, 0.15)`);
    grad.addColorStop(1, `hsla(${hue}, 80%, 60%, 0)`);
    ctx.fillStyle = grad;
    ctx.beginPath();
    ctx.moveTo(cx - beamWidth * 0.15, baseY);
    ctx.lineTo(cx - beamWidth, baseY - beamHeight);
    ctx.lineTo(cx + beamWidth, baseY - beamHeight);
    ctx.lineTo(cx + beamWidth * 0.15, baseY);
    ctx.closePath();
    ctx.fill();
  });
  ctx.restore();
