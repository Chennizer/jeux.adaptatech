...
export function createWinterScene(p) {
  ...
  const drawMoon = () => {
    ...
    const radius = Math.min(p.width, p.height) * 0.06;
    p.noStroke();

    // Halo
    const haloSteps = 4;
    for (let i = haloSteps; i >= 1; i -= 1) {
      const t = i / haloSteps;
      const alpha = 40 * t;
      p.fill(210, 226, 244, alpha);
      p.circle(moonX, moonY, radius * (1.4 + t * 0.5));
    }

    // Core
    p.fill(234, 240, 248, 240);
    p.circle(moonX, moonY, radius);

    // Crescent accent
    p.fill(210, 220, 230, 200);
    p.circle(moonX + radius * 0.2, moonY - radius * 0.05, radius * 0.72);
  };
