document.addEventListener('DOMContentLoaded', () => {
  ...
  function stopPreview() {
    if (currentPreview) {
      if (currentPreview === 'youtube') {
        try { if (youtubePlayer) youtubePlayer.stopVideo(); } catch {}
        if (!videoPlaying) {
    ...
    return Promise.all(
      videoUrls.map(url => {
        return new Promise(resolve => {
          if (typeof isYouTubeUrl === 'function' && isYouTubeUrl(url)) {
            loadedCount++;
            if (loadingIndicator) {
  ...
  function playVideo(videoUrl) {
    ...
    if (typeof isYouTubeUrl === 'function' && isYouTubeUrl(videoUrl)) {
      videoPlayer.style.display = 'none';
      if (youtubeDiv) youtubeDiv.style.display = 'block';
      const vid = (typeof getYouTubeId === 'function') ? getYouTubeId(videoUrl) : null;
      if (!youtubePlayer) {
      ...
      videoTimeLimitTimeout = setTimeout(() => {
        if (videoPlaying) {
          if (enableResumeVideoCheckbox.checked) {
            if (typeof isYouTubeUrl === 'function' && isYouTubeUrl(videoUrl) && youtubePlayer && youtubePlayer.getCurrentTime) {
          ...
          if (typeof isYouTubeUrl === 'function' && isYouTubeUrl(videoUrl) && youtubePlayer) {
