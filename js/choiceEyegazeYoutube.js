document.addEventListener('DOMContentLoaded', () => {
  ...
  let pointerGuardAnchor = null;

  function armPointerGuard() {
    requirePointerMotion = true;
    pointerGuardAnchor = lastPointerPosition ? { ...lastPointerPosition } : null;
  }
  ...
  document.addEventListener('pointermove', event => {
    ...
    let skipNormalHandling = false;

    if (requirePointerMotion) {
      if (!pointerGuardAnchor) {
        pointerGuardAnchor = previousPosition
          ? { ...previousPosition }
          : { x: clientX, y: clientY };
        if (!previousPosition) {
          return;
      ...
      const dx = clientX - pointerGuardAnchor.x;
      const dy = clientY - pointerGuardAnchor.y;

      if (Math.hypot(dx, dy) >= POINTER_MOVE_THRESHOLD) {
        requirePointerMotion = false;
        pointerGuardAnchor = null;
    ...
    if (!skipNormalHandling && !videoPlaying) {
      const tile = targetElement ? targetElement.closest('.tile') : null;
      if (
        tile &&
        tileChoiceMap.has(tile) &&
