...
export function createSummerScene(p) {
  ...
  function populateHerbFloor() {
    herbFloor.length = 0;
    const count = Math.floor(p.width / 20) + 120;
    for (let i = 0; i < count; i += 1) {
      const depth = p.random(0, 1);
      const x = p.random(-60, p.width + 60);
      const baseY = p.lerp(p.height * 0.7, p.height * 0.99, depth) + p.random(-8, 10);
      const height = p.lerp(p.height * 0.22, p.height * 0.38, 1 - depth) * p.random(0.9, 1.15);
  ...
  function drawGrass(stillness, movement) {
    ...
    const layered = [...grass, ...meadowPlants, ...herbFloor];
