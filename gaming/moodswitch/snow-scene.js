...
class SnowGlitter {
  ...
  reset() {
    ...
    this.size = p.random(2.2, 4.8);
    this.life = p.random(110, 180);
    this.vx = p.random(-0.3, 0.3);
    this.vy = p.random(-0.2, 0.1);
    this.alpha = p.random(150, 230);
    this.twinkleSpeed = p.random(0.05, 0.14);
    this.phase = p.random(p.TWO_PI);
  }
...
export function createSnowScene(p) {
  ...
  function spawnGlitter({ reset = true } = {}) {
    ...
    const base = Math.floor(area * 0.000025 * 3) + 24 * 3;
    const count = reset ? base : Math.ceil(base * 0.65);
