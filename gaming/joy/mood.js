...
import { createSunbeamScene } from './sunbeam-scene.js';
import { createStreamerScene } from './streamer-scene.js';
...
function ensureSceneAudio(sceneId) {
  ...
  if (!sceneAudioCache.has(sceneId)) {
    const audio = createAudioElement(SCENE_SOUNDTRACKS[sceneId], { volume: 0.5, loop: true });
...
const sketch = p => {
  p.setup = () => {
    ...
    scenes = [
      ...
      createSunbeamScene(p),
      createStreamerScene(p)
    ];
