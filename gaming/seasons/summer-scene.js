...
export function createSummerScene(p) {
  ...
  function populateGoldenHerbs() {
    ...
    const count = Math.floor(p.width / 28) + 60;
    for (let i = 0; i < count; i += 1) {
      const depth = p.random(0, 0.6);
      const x = p.random(-50, p.width + 50);
      const baseY = p.height * 0.82 + (p.noise(groundSeed + 500 + x * 0.0008) - 0.5) * 50;
      const height = p.lerp(p.height * 0.16, p.height * 0.28, 1 - depth) * p.random(0.9, 1.15);
      goldenHerbs.push(new MeadowPlant(p, x, baseY, height, depth, {
        stem: p.lerpColor(p.color(palette.meadowLight), p.color(palette.meadowDeep), depth * 0.8),
        headWarm: p.color(...palette.accentWarm),
        headCool: p.color(...palette.accentCool)
      }));
    }
  ...
  function drawGrass(stillness, movement) {
    ...
    const layered = [...goldenHerbs, ...grass, ...meadowPlants];
