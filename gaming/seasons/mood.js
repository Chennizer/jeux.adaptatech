...
const SCENE_SOUNDTRACKS = {
  ...
  summer: '../../sounds/sadness/softrain.mp3',
  autumn: '../../sounds/sadness/candle.mp3',
  winter: '../../sounds/sadness/windandsnow.mp3'
};
...
const SLOW_BASE_SPEED = 0.22;
const SLOW_PEAK_SPEED = 1.3;
const SPEED_PHASES = {
  rampUp: 2_000,
  hold: 2_200,
  rampDown: 2_000
...
function getCurrentSpeedMultiplier() {
  ...
  const localDuration = SPEED_PHASES.rampUp + SPEED_PHASES.hold + SPEED_PHASES.rampDown;
  const relative = elapsed % localDuration;

  let base = SLOW_BASE_SPEED;
  if (relative < SPEED_PHASES.rampUp) {
    base = lerp(SLOW_BASE_SPEED, SLOW_PEAK_SPEED, relative / SPEED_PHASES.rampUp);
  } else if (relative < SPEED_PHASES.rampUp + SPEED_PHASES.hold) {
    base = SLOW_PEAK_SPEED;
  } else {
    const rampDownElapsed = relative - SPEED_PHASES.rampUp - SPEED_PHASES.hold;
    base = lerp(SLOW_PEAK_SPEED, SLOW_BASE_SPEED, rampDownElapsed / SPEED_PHASES.rampDown);
  }
  ...
  const elapsedBurst = now - speedBurst.start;
  if (elapsedBurst > 1400) {
    speedBurst = null;
    return base;
  }
  const t = clamp01(elapsedBurst / 1400);
  const burst = lerp(1.5, 1, t);
  return base * burst;
}
