...
class RainSparkle {
  ...
  reset() {
    ...
    this.radius = p.random(3, 7);
    this.life = p.random(70, 140);
    this.vx = p.random(-0.25, 0.25);
    this.vy = p.random(0.15, 0.4);
    this.alpha = p.random(160, 230);
  }
...
export function createRainScene(p) {
  ...
  function spawnSparkles({ reset = true } = {}) {
    ...
    const base = Math.floor(area * 0.00003 * 3) + 18 * 3;
    const count = reset ? base : Math.ceil(base * 0.6);
