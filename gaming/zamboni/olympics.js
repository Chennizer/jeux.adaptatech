...
function spawnFirework(){
  ...
  const y = vh * (0.12 + Math.random() * 0.35);
  const colors = ['#ff5252','#ffd740','#40c4ff','#69f0ae','#ff80ab', '#b388ff'];
  const color = colors[openingStage.ringPresses % colors.length];
  const count = 180;
  const baseSpeed = 1.8 + Math.random() * 2.8;
  for(let i=0;i<count;i++){
    const angle = Math.random() * Math.PI * 2;
    const speed = baseSpeed * (0.7 + Math.random() * 1.2);
...
function drawOpeningStage(now){
  ...
  openingStage.stars.forEach(star => {
    const twinkle = 0.5 + Math.sin(now * 0.0015 + star.twinkle) * 0.5;
  ...
  openingStage.fireworks.forEach(p => {
    ...
    const alpha = Math.max(0, (1 - t) * (0.8 - 0.4 * t));
    ctx.strokeStyle = hexToRgba(p.color, alpha);
    ctx.lineWidth = Math.max(1, p.size * (1 - t));
    ctx.beginPath();
    ctx.moveTo(p.px, p.py);
    ctx.lineTo(p.x, p.y);
    ctx.stroke();

    ctx.fillStyle = hexToRgba(p.color, alpha * 0.6);
    ctx.beginPath();
  ...
  if(openingStage.complete){
    ...
    ctx.lineWidth = 6;
    ctx.beginPath();
    ctx.arc(vw/2, vh/2, Math.min(vw, vh) * 0.26 * (1 + pulse * 0.15), 0, Math.PI * 2);
    ctx.stroke();
    ctx.restore();
  }
}
...
function spawnSnowBurst(){
  ...
  for(let i=0;i<180;i++){
    const angle = (-Math.PI / 4) + Math.random() * (Math.PI / 2.2);
...
function drawProjectorLights(vw, vh, now){
  const baseY = vh * 0.92;
  const beamHeight = vh * 0.5;
  const beams = [
    {x: vw * 0.15, color: 'rgba(80,180,255,0.2)'},
    {x: vw * 0.4, color: 'rgba(255,160,100,0.18)'},
    {x: vw * 0.65, color: 'rgba(180,120,255,0.2)'},
    {x: vw * 0.85, color: 'rgba(255,220,120,0.16)'}
  ];

  ctx.save();
  ctx.globalCompositeOperation = 'screen';
  beams.forEach((beam, index) => {
    const sway = Math.sin(now * 0.0006 + index) * vw * 0.02;
    const width = vw * 0.22;
    ctx.fillStyle = beam.color;
    ctx.beginPath();
    ctx.moveTo(beam.x - width * 0.5 + sway, baseY);
    ctx.lineTo(beam.x + width * 0.5 + sway, baseY);
    ctx.lineTo(beam.x + sway * 0.4, baseY - beamHeight);
    ctx.closePath();
    ctx.fill();
  });
  ctx.restore();
