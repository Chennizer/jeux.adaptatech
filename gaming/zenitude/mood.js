import { createLotusPondScene } from './lotus-pond-scene.js';
import { createAuroraSkyScene } from './aurora-sky-scene.js';
import { createLanternDriftScene } from './lantern-drift-scene.js';
import { createMountainMistScene } from './mountain-mist-scene.js';
import { createZenGardenScene } from './zen-garden-scene.js';
...
const SCENE_SOUNDTRACKS = {
  lotus: '../../sounds/sadness/subtlewind.mp3',
  aurora: '../../sounds/sadness/windandsnow.mp3',
  lanterns: '../../sounds/sadness/candle.mp3',
  mountain: '../../sounds/sadness/softrain.mp3',
  garden: '../../sounds/sadness/softrain.mp3'
};
...
const SLOW_BASE_SPEED = 0.18;
const SLOW_PEAK_SPEED = 1.38;
const SPEED_PHASES = {
  rampUp: 2_400,
  hold: 2_400,
  rampDown: 2_400
...
function getCurrentSpeedMultiplier() {
  const base = mode === MODE_SLOW ? SLOW_BASE_SPEED : 1;
  if (!speedBurst) {
    return base;
  }
  const elapsed = performance.now() - speedBurst.start;
...
const sketch = p => {
  p.setup = () => {
    ...
    scenes = [
      createLotusPondScene(p),
      createAuroraSkyScene(p),
      createLanternDriftScene(p),
      createMountainMistScene(p),
      createZenGardenScene(p)
    ];
