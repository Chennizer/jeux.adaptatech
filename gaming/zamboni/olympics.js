...
function spawnSnowBurst(){
  ...
  snowStage.streams.push({
    startX: cannon.x + cannon.w * 0.7,
    startY: cannon.y + cannon.h * 0.2,
    duration: 1000,
    age: 0,
  });
}
...
function updateSnowParticles(){
  snowStage.streams.forEach(stream => {
    const emission = 14;
    for(let i=0;i<emission;i++){
      const angle = (-Math.PI / 3.2) + Math.random() * (Math.PI / 2.0);
      const speed = 3.5 + Math.random() * 5.5;
      snowStage.particles.push({
        x: stream.startX + Math.random() * 8,
        y: stream.startY + Math.random() * 8,
        vx: Math.cos(angle) * speed,
        vy: Math.sin(angle) * speed,
        life: 0,
  ...
  snowStage.streams = snowStage.streams.filter(stream => stream.age < stream.duration);
...
function drawSnowStream(){
  const cannon = getCannonBounds();
  const startX = cannon.x + cannon.w * 0.7;
  const startY = cannon.y + cannon.h * 0.2;
  const endX = startX + cannon.w * 2.4;
  const endY = startY - cannon.h * 1.8;

  snowStage.streams.forEach(stream => {
    const intensity = 1 - (stream.age / stream.duration);
    const grad = ctx.createLinearGradient(startX, startY, endX, endY);
    grad.addColorStop(0, `rgba(255,255,255,${0.55 * intensity})`);
    grad.addColorStop(0.5, `rgba(255,255,255,${0.25 * intensity})`);
    grad.addColorStop(1, 'rgba(255,255,255,0)');
    ctx.save();
    ctx.globalCompositeOperation = 'screen';
    ctx.fillStyle = grad;
    ctx.beginPath();
    ctx.moveTo(startX - 14, startY + 6);
    ctx.lineTo(endX + 60, endY - 40);
    ctx.lineTo(endX - 20, endY - 80);
