...
export function createSummerScene(p) {
  ...
  function populateGrass() {
    grass.length = 0;
    const count = Math.floor(Math.max(320, p.width / 2.6));
    for (let i = 0; i < count; i += 1) {
  ...
  function populateHerbFloor() {
    ...
    for (let r = 0; r < rows; r += 1) {
      ...
      const depthBase = p.constrain(p.lerp(0.12, 0.92, rowMix) + p.random(-0.08, 0.08), 0, 1);
      const baseY = p.lerp(p.height * 0.52, p.height * 0.99, rowMix) + p.random(-6, 6);
      for (let c = 0; c < cols; c += 1) {
        const depth = p.constrain(depthBase + p.random(-0.05, 0.05), 0, 1);
        ...
        herbFloor.push(new MeadowPlant(p, x, baseY + p.random(-10, 10), height, depth, {
