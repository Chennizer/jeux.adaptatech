...
class GroundSparkle {
  ...
  reset() {
    ...
    this.y = p.random(this.horizonY + p.height * 0.02, p.height - p.height * 0.04);
    this.size = p.random(1.2, 2.4);
    this.base = p.random(120, 170);
  ...
  draw() {
    ...
    const alpha = p.map(twinkle, 0, 1, this.base - 30, this.base + 60, true);
    p.noStroke();
...
export function createWinterScene(p) {
  ...
  function resize() {
    ...
    sparkles.length = 0;
    const sparkleCount = Math.max(40, Math.floor((p.width * (p.height - horizonY)) / 12000));
    for (let i = 0; i < sparkleCount; i += 1) {
      sparkles.push(new GroundSparkle(p, horizonY));
    }
  }
