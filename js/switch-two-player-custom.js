...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function refreshCategoryButtons() {
    if (!urlVideoList) return;
    urlVideoList.querySelectorAll('.category-actions').forEach(node => node.remove());
    const state = getCategoryState();
    if (!isCategoriesEnabled(state) || !state || !Array.isArray(state.categories)) return;
    ...
    cards.forEach(card => {
      ...
      state.categories.forEach(cat => {
        ...
        btn.title = `Déplacer vers ${cat.name}`;
  ...
  function moveCardToCategory(card, targetId) {
    syncCategoriesFromDom();
    const state = getCategoryState();
    if (!state || !Array.isArray(state.categories)) return;
    const active = getActiveCategory(state);
    const target = state.categories.find(cat => cat.id === targetId);
    if (!active || !target) return;
    const url = card.dataset.src;
    if (!url) return;
    active.urls = Array.isArray(active.urls) ? active.urls.filter(u => u !== url) : [];
