...
class Dragonfly {
  ...
  reset(side) {
    const p = this.p;
    this.side = side || (p.random() > 0.5 ? 'left' : 'right');
    this.x = this.side === 'left' ? -30 : p.width + 30;
    this.y = p.random(this.horizonY - 40, this.horizonY + 90);
    this.speed = p.random(2.4, 3.6);
    this.bobSeed = p.random(1000);
    this.pitch = p.random(-0.02, 0.02);
    this.wingPhase = p.random(p.TWO_PI);
    this.trail.length = 0;
  }

  update(multiplier, excitement) {
    const p = this.p;
    const dir = this.side === 'left' ? 1 : -1;
    const dt = Math.min(32, p.deltaTime || 16) / 16;
    const lively = 1 + excitement * 0.8;

    this.x += this.speed * dir * lively * dt;
    this.y += Math.sin(p.frameCount * 0.06 + this.bobSeed) * 1.6;
    this.y += (p.noise(this.bobSeed, p.frameCount * 0.008) - 0.5) * 1.4;
    this.pitch += p.random(-0.003, 0.003) * lively;
    this.pitch = p.constrain(this.pitch, -0.08, 0.08);

    this.wingPhase += 0.4 * lively;
    const wobble = Math.sin(this.wingPhase) * 6;
    this.trail.unshift({ x: this.x - dir * 8, y: this.y + wobble * 0.2, alpha: 80 });
    if (this.trail.length > 10) this.trail.pop();

    if ((this.side === 'left' && this.x > p.width + 40) || (this.side === 'right' && this.x < -40)) {
      this.reset(this.side === 'left' ? 'right' : 'left');
    }
  }
  ...
  draw(excitement) {
    ...
    for (let i = 0; i < this.trail.length; i += 1) {
      ...
      p.stroke(220, 240, 220, t.alpha * p.map(excitement, 0, 1, 0.6, 1));
      p.strokeWeight(2);
      p.point(t.x - this.x, t.y - this.y);
    }

    const bodyColor = p.color(52, 128, 126, 200);
    const wingAlpha = p.map(Math.sin(this.wingPhase), -1, 1, 80, 180);
    p.strokeWeight(3);
    p.stroke(bodyColor);
    p.line(-8, 0, 12, 0);
    p.strokeWeight(2);
    ...
    p.noFill();
    p.stroke(200, 230, 220, wingAlpha);
    p.strokeWeight(4);
    p.bezier(0, 0, -6, -8, -4, -16, 10, -8);
    p.bezier(0, 0, -6, 8, -4, 16, 10, 8);
    p.pop();
  }
}
...
class SeedWhorl {
  constructor(p, x, y, palette) {
    ...
    this.radius = 6;
    this.angle = p.random(p.TWO_PI);
    this.angularVelocity = p.random(0.05, 0.12);
    this.growth = p.random(0.5, 1.1);
    this.life = 180;
  }

  update(multiplier, stillness) {
    const p = this.p;
    this.angle += this.angularVelocity * (0.7 + multiplier);
    this.radius += (0.8 + stillness * 0.8) * multiplier;
    this.life -= 1.6 * multiplier;
  }

  draw() {
    const p = this.p;
    if (this.life <= 0) return false;
    const alpha = p.map(this.life, 0, 180, 0, 140);
    p.push();
    p.translate(this.x, this.y);
    const seeds = 6;
    for (let i = 0; i < seeds; i += 1) {
      const theta = this.angle + (p.TWO_PI / seeds) * i;
      const r = this.radius * p.lerp(0.5, 1.15, this.life / 180);
      const sx = Math.cos(theta) * r;
      const sy = Math.sin(theta) * r * 0.8;
      p.noStroke();
      p.fill(this.palette.shadow[0], this.palette.shadow[1], this.palette.shadow[2], alpha);
      p.ellipse(sx, sy, 6, 10);
      p.fill(255, 240, 200, alpha * 0.9);
      p.ellipse(sx + 1, sy - 2, 4, 8);
    }
    p.pop();
    return true;
  }
...
export function createSummerScene(p) {
  ...
  function populateDragonflies(horizonY) {
    ...
    const count = Math.max(3, Math.floor(p.width / 240));
    for (let i = 0; i < count; i += 1) {
      const flyer = new Dragonfly(p, horizonY);
      flyer.x += p.random(-p.width * 0.5, p.width * 0.5);
      dragonflies.push(flyer);
    }
  }
  ...
  function spawnWhorl(x, y) {
    whorls.push(new SeedWhorl(p, x, y, palette));
  }
  ...
  return {
    ...
    pulse() {
      ...
      spawnLeaves(10);
      for (let i = 0; i < 2; i += 1) {
        spawnWhorl(playerX + p.random(-40, 40), groundY(playerX) - p.random(40, 80));
      }
    },
    draw() {
      const stillness = p.constrain(1.2 - speedMultiplier, 0, 1);
      const movement = p.constrain((speedMultiplier - 0.25) / 1.2, 0, 1);
      const excitement = p.map(movement, 0, 1, 0.1, 1.1) + p.map(stillness, 0, 1, 0, 0.25);
      ...
      if (p.random() < 0.01 * (1 + movement * 1.5)) {
        const launchX = p.lerp(playerX, p.width * 0.65, Math.random());
        spawnWhorl(launchX + p.random(-40, 40), groundY(launchX) - p.random(60, 120));
      }
      ...
      for (let i = whorls.length - 1; i >= 0; i -= 1) {
        whorls[i].update(speedMultiplier, stillness);
        const alive = whorls[i].draw();
        if (!alive || whorls[i].life <= 0) {
          whorls.splice(i, 1);
        }
      }
      ...
      dragonflies.forEach(flyer => {
        flyer.update(speedMultiplier, excitement);
        flyer.draw(excitement);
      });
