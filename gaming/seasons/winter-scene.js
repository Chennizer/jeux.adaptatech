...
export function createWinterScene(p) {
  ...
  const drawAurora = () => {
    const baseY = p.height * 0.18;
    const amplitude = p.height * 0.04;
    p.noStroke();
    p.fill(140, 210, 240, 36);
    p.beginShape();
    for (let x = -20; x <= p.width + 20; x += 12) {
      const n = p.noise(x * 0.0045, 120);
      const y = baseY + Math.sin((x / p.width) * p.TWO_PI) * amplitude * 0.25 - n * amplitude;
      p.vertex(x, y);
    }
    for (let x = p.width + 20; x >= -20; x -= 12) {
      const n = p.noise(x * 0.0045, 220);
      const y = baseY + p.height * 0.08 + Math.cos((x / p.width) * p.PI) * amplitude * 0.2 - n * amplitude * 0.5;
      p.vertex(x, y);
    }
    p.endShape(p.CLOSE);

    p.fill(110, 190, 230, 28);
    p.beginShape();
    for (let x = -20; x <= p.width + 20; x += 14) {
      const n = p.noise(x * 0.0038, 320);
      const y = baseY + p.height * 0.04 + Math.sin((x / p.width) * p.TWO_PI * 0.8) * amplitude * 0.35 - n * amplitude * 0.6;
      p.vertex(x, y);
    }
    for (let x = p.width + 20; x >= -20; x -= 14) {
      const n = p.noise(x * 0.0038, 420);
      const y = baseY + p.height * 0.11 + Math.cos((x / p.width) * p.PI * 0.9) * amplitude * 0.25 - n * amplitude * 0.4;
      p.vertex(x, y);
    }
    p.endShape(p.CLOSE);
  };
