document.addEventListener('DOMContentLoaded', () => {
    ...
    function preloadMedia(list, onComplete) {
        ...
        const loadingBar = document
          .getElementById('control-panel-loading-bar-container')
          .querySelector('#control-panel-loading-bar')
    ...
    function setPromptVisibility(show) {
        ...
        // If we are making the prompt visible => replay the sound
        if (show) {
    ...
    function showPromptForCurrentPlayer() {
        clearTimedCycle()
        // (Removed the direct call to playSoundPromptForPlayer() here,
        // so we only do it in setPromptVisibility(true).)
        ...
        if (!timedOn) {
            // just show the prompt (which also replays sound in setPromptVisibility(true))
            setPromptVisibility(true)
            return
        }
    ...
    function loadConfig() {
        // Build image prompts for Player 1
        spacePromptImages.forEach((p) => {
            const card = document.createElement('div')
            card.className = 'image-card'
            card.dataset.src = p.src
            card.innerHTML = `<img src="${p.src}" alt="${p.alt}">`
            imageCardsContainer1.appendChild(card)

            card.addEventListener('click', () => {
                textPromptInput.value = ''
                useTextPrompt = false
                Array.from(imageCardsContainer1.querySelectorAll('.image-card'))
        ...
        miscOptions.forEach(o => {
            if (o.id === 'timed-prompt-option') {
                ...
                numericDiv.classList.add('misc-option-wrapper')
                numericDiv.id = 'timed-seconds-wrapper'
                numericDiv.style.display = 'none'

                const numericLabel = document.createElement('label')
                numericLabel.textContent = 'Temps pour appuyer sur la switch (s)'
                numericLabel.style.color = 'black'

                const numberInput = document.createElement('input')
                numberInput.type = 'number'
                numberInput.min = '1'
                numberInput.value = o.label || '5'
                ...
                numericDiv.appendChild(numericLabel)
                numericDiv.appendChild(numberInput)
                miscOptionsContainer.appendChild(numericDiv)
    ...
    // Function to hide the first option on focus
    ...
    // Function to show the first option on blur if no selection made
    function showPlaceholderOnBlur(selectElement) {
      selectElement.addEventListener('blur', function() {
        const firstOption = this.options[0];
        // Restore display if no valid selection made
