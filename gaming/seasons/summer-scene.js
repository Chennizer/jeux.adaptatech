class GrassBlade {
  ...
  draw({ playerX, movement, stillness, shadowTint, gust }) {
    const p = this.p;
    ...
    const gustPush = gust.strength * gust.direction * p.lerp(this.height * 0.08, this.height * 0.16, 1 - this.depth);

    const tipX = this.x + sway + breeze + parting + gustPush;
...
class SeedDrift {
  constructor(p, originX, originY, gust) {
    this.p = p;
    this.reset(originX, originY, gust);
  }

  reset(originX, originY, gust) {
    const p = this.p;
    this.x = originX + p.random(-30, 30);
    this.y = originY + p.random(-10, 10);
    this.vx = (p.random(0.2, 0.6) + gust.strength * 0.6) * gust.direction;
    this.vy = p.random(-0.2, 0.05);
    this.spin = p.random(0.02, 0.04) * p.random([1, -1]);
    this.angle = p.random(p.TWO_PI);
    this.life = p.random(160, 240);
    this.size = p.random(4, 7);
    this.tint = p.color(240, 220, 180, 160);
  }

  update(gust) {
    const p = this.p;
    this.x += this.vx * (0.6 + gust.strength * 0.8);
  ...
  draw(shadowTint) {
    ...
    p.rotate(this.angle * 0.2);
    p.noStroke();
    p.fill(this.tint);
    p.ellipse(0, 0, this.size * 0.9, this.size * 1.4);
    p.fill(shadowTint.levels[0], shadowTint.levels[1], shadowTint.levels[2], 70);
    p.ellipse(-this.size * 0.2, this.size * 0.1, this.size * 0.5, this.size * 0.3);
    p.pop();
    return true;
  }
}
...
export function createSummerScene(p) {
  ...
  let playerX = 0;
  let lastTouchAt = 0;
  let gust = { strength: 0, direction: 1, nextAt: 0 };
  ...
  function spawnSeeds(count = 12, originX = p.width * 0.6, originY = p.height * 0.46) {
    for (let i = 0; i < count; i += 1) {
      seeds.push(new SeedDrift(p, originX, originY, gust));
    }
  }

  function triggerGust(reason = 'ambient') {
    const now = p.millis();
    if (now < gust.nextAt && reason === 'ambient') return;

    gust.direction = p.random([-1, 1]);
    gust.strength = p.random(0.5, 1);
    gust.nextAt = now + p.random(7000, 12000);
    spawnSeeds(p.floor(p.random(8, 16)), p.width * p.random(0.35, 0.65), p.height * 0.45);
  }
  ...
  function resize() {
    ...
    leaves.length = 0;
    seeds.length = 0;
    gust = { strength: 0, direction: 1, nextAt: p.millis() + 3000 };
  }
  ...
  function drawGrass(stillness, movement) {
    ...
    grass.forEach(blade => {
      blade.draw({ playerX, movement, stillness, shadowTint, gust });
    });
  }
  ...
  return {
    ...
    draw() {
      ...
      if (p.frameCount % 300 === 0 && p.random() < 0.5) {
        triggerGust();
      }
      ...
      for (let i = seeds.length - 1; i >= 0; i -= 1) {
        seeds[i].update(gust);
        const alive = seeds[i].draw(p.color(...palette.shadow));
        if (!alive || seeds[i].x < -40 || seeds[i].x > p.width + 40 || seeds[i].y > p.height + 20) {
          seeds.splice(i, 1);
        }
      }
      ...
      gust.strength = p.lerp(gust.strength, 0, 0.01);
      shimmer = p.lerp(shimmer, 0, 0.01 * speedMultiplier + gust.strength * 0.002);
    }
  };
}
