...
function setStage(index){
  ...
  if(index === 1){
    snowStage.presses = 0;
    snowStage.particles = [];
    snowStage.streamTime = 0;
    snowStage.lastTime = 0;
...
function spawnFirework(){
  ...
  const y = vh * (0.12 + Math.random() * 0.38);
  const styleRoll = Math.random();
  const style = styleRoll < 0.2 ? 'ring' : (styleRoll < 0.35 ? 'willow' : 'peony');
  spawnFireworkBurst(x, y, 1.15, style);
}
...
function buildStars(){
  const vw = window.innerWidth;
  const vh = window.innerHeight;
  const count = Math.floor((vw * vh) / 16000);
  openingStage.stars = Array.from({length: count}, () => ({
    x: Math.random() * vw,
    y: Math.random() * vh * 0.85,
...
function drawStars(now){
  ...
  openingStage.stars.forEach(star => {
    const flicker = 0.6 + Math.sin(now * 0.001 * star.speed + star.twinkle) * 0.4;
...
function drawProjectorLights(){
  const vw = window.innerWidth;
  const vh = window.innerHeight;
  const baseY = vh * 0.92;
  const lights = [
    {x: vw * 0.18, width: vw * 0.22, hue: 200},
    {x: vw * 0.5, width: vw * 0.28, hue: 320},
    {x: vw * 0.82, width: vw * 0.22, hue: 40},
  ];

  ctx.save();
  ctx.globalCompositeOperation = 'screen';
  lights.forEach(light => {
    const grad = ctx.createLinearGradient(light.x, baseY, light.x, vh * 0.1);
    grad.addColorStop(0, `hsla(${light.hue},80%,65%,0.35)`);
    grad.addColorStop(1, 'rgba(0,0,0,0)');
    ctx.fillStyle = grad;
    ctx.beginPath();
    ctx.moveTo(light.x - light.width * 0.5, baseY);
    ctx.lineTo(light.x + light.width * 0.5, baseY);
    ctx.lineTo(light.x, vh * 0.1);
    ctx.closePath();
    ctx.fill();
  });
  ctx.restore();
...
function spawnFireworkBurst(x, y, power=1, style='peony'){
  const baseCount = 80 + Math.floor(80 * power);
  const count = Math.floor(baseCount * (0.9 + Math.random() * 0.4));
  const sizeFactor = 1 + power * 0.3;
  ...
  for(let i=0;i<count;i++){
    ...
    if(style === 'ring'){
      const ringSpeed = (2.2 + Math.random() * 1.5) * sizeFactor;
      vx = Math.cos(angle) * ringSpeed;
      vy = Math.sin(angle) * ringSpeed;
    }else if(style === 'willow'){
      const willowSpeed = (1.8 + Math.random() * 1.4) * sizeFactor;
      vx = Math.cos(angle) * willowSpeed;
      vy = Math.sin(angle) * willowSpeed * 0.6 - (1.2 + 0.8 * sizeFactor);
    }

    const hue = (fireworkHueBase + (Math.random() * 40 - 20) + i) % 360;
    openingStage.fireworks.push({
      x, y, px: x, py: y,
      vx, vy,
      drag: 0.986,
      gy: 0.04,
      life: (0.8 + Math.random() * 0.6) * sizeFactor,
      age: 0,
      size: (1.4 + Math.random() * 1.6) * sizeFactor,
      hue,
...
function updateFireworks(dt){
  openingStage.fireworks = openingStage.fireworks.filter(p => p.age < p.life);
...
function drawOpeningStage(now, dt){
  const vw = window.innerWidth;
  const vh = window.innerHeight;
  ...
  fireworkHueBase = (fireworkHueBase + 45 * dt) % 360;
  updateFireworks(dt);
  ctx.save();
  ...
  openingStage.fireworks.forEach(p => {
    ...
    const alpha = Math.max(0, (1 - t) * (0.7 - 0.25 * t));
    ...
    const glowR = Math.max(1, p.size * 2.1);
    ctx.beginPath();
    ctx.arc(p.x, p.y, glowR * 0.5, 0, Math.PI * 2);
    ctx.fill();
  });
  ctx.restore();
...
function drawOlympicRings(now){
  const vw = window.innerWidth;
  const vh = window.innerHeight;
  const ringSize = Math.min(vw, vh) * 0.145;
  const gap = ringSize * 0.25;
  const centerX = vw / 2;
  const centerY = vh / 2;
...
function spawnSnowBurst(){
  ...
  for(let i=0;i<160;i++){
    const angle = (-Math.PI / 4) + Math.random() * (Math.PI / 2.6);
  ...
  snowStage.streamTime = Math.max(snowStage.streamTime, 1200);
}

function spawnSnowStream(dt){
  if(snowStage.streamTime <= 0) return;
  const cannon = getCannonBounds();
  const startX = cannon.x + cannon.w * 0.75;
  const startY = cannon.y + cannon.h * 0.2;
  const count = Math.floor(80 * dt * 60);
  for(let i=0;i<count;i++){
    const angle = (-Math.PI / 4) + Math.random() * (Math.PI / 2.8);
  ...
  snowStage.streamTime = Math.max(0, snowStage.streamTime - dt * 1000);
}
...
function drawSnowStage(){
  ...
  const dt = snowStage.lastTime ? (now - snowStage.lastTime) / 1000 : 0.016;
  snowStage.lastTime = now;
  spawnSnowStream(dt);
  updateSnowParticles();
  ctx.save();
...
function draw(){
  const now = performance.now();
  const dt = (window.__lastOlympicFrameTime ? (now - window.__lastOlympicFrameTime) : 16) / 1000;
  window.__lastOlympicFrameTime = now;
  const vw = window.innerWidth;
  const vh = window.innerHeight;
  ctx.clearRect(0,0,vw,vh);

  if(stageState.index === 0){
    drawOpeningStage(now, dt);
  }
  if(stageState.index === 1){
    drawSnowStage();
  }
