document.addEventListener('DOMContentLoaded', () => {
  ...
  function clearMenuMusicRetryTimer() {
    if (menuMusicRetryTimer) {
      clearInterval(menuMusicRetryTimer);
      menuMusicRetryTimer = null;
    }
  }

  function startMenuMusicAutoplayRetryLoop() {
    if (!menuMusicAudio || menuMusicRetryTimer) {
      return;
    }
    ...
    const tryStart = () => {
      ...
      if (isAudioPlaying(menuMusicAudio)) {
        clearMenuMusicRetryTimer();
        return;
      }
    ...
    tryStart();
    menuMusicRetryTimer = setInterval(tryStart, 1500);
  }
  ...
  function autoStartMenuMusicAfterLoad() {
    ...
    startMenuMusicAutoplayRetryLoop();
  }
