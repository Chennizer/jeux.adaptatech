...
function spawnFirework(){
  ...
  const baseSpeed = type === 'willow' ? 4.1 : 5.6;
  ...
  for(let i=0;i<count;i++){
    ...
    openingStage.fireworks.push({
      ...
      life: 0,
      ttl: type === 'willow' ? 230 + Math.random() * 60 : 180 + Math.random() * 70,
      hue: (hue + hueShift + 360) % 360,
      sat: type === 'willow' ? 65 + Math.random() * 30 : 85 + Math.random() * 15,
      size: type === 'ring' ? 5.2 : 4.2 + Math.random() * 3.2,
      glow: type === 'willow' ? 1 : Math.random() > 0.7 ? 1 : 0,
      twinkle: 0.5 + Math.random() * 0.6,
      drag: type === 'willow' ? 0.994 : 0.992,
      gravity: type === 'willow' ? 0.055 : 0.08,
    });
  }
}
...
function drawOpeningStage(now){
  ...
  openingStage.fireworks.forEach(p => {
    ...
    const flicker = 0.8 + p.twinkle * Math.sin((p.x + p.y + now) * 0.012);
    if(p.glow){
      ctx.fillStyle = `hsla(${p.hue},${p.sat}%,70%,${alpha * 0.45})`;
      ctx.beginPath();
      ctx.arc(p.x, p.y, p.size * 3, 0, Math.PI * 2);
      ctx.fill();
    }
    ctx.fillStyle = `hsla(${p.hue},${p.sat}%,72%,${alpha * flicker})`;
    ctx.beginPath();
...
function drawOlympicRings(now){
  const vw = window.innerWidth;
  const vh = window.innerHeight;
  const ringSize = Math.min(vw, vh) * 0.19;
  const gap = ringSize * 0.35;
  const centerX = vw / 2;
  const centerY = vh / 2.08;
...
function getMountainYAt(x){
  const vw = window.innerWidth;
  const vh = window.innerHeight;
  const progress = snowStage.presses / 5;
  const peakHeight = vh * (0.2 + progress * 0.35);
  const baseY = vh * sceneConfig.snowMountain.baseYRatio;
  const centerX = vw * sceneConfig.snowMountain.centerXRatio;
  const baseWidth = vw * sceneConfig.snowMountain.baseWidthRatio;
  const leftBaseX = centerX - baseWidth * sceneConfig.snowMountain.leftRatio;
  const rightBaseX = centerX + baseWidth * sceneConfig.snowMountain.rightRatio;
  const peakX = centerX;
  const peakY = baseY - peakHeight;
  if(x < leftBaseX || x > rightBaseX) return vh + 100;
  if(x <= peakX){
    const t = (x - leftBaseX) / (peakX - leftBaseX);
    return baseY - t * peakHeight;
  }
  const t = (x - peakX) / (rightBaseX - peakX);
...
function updateSnowParticles(now){
  ...
  snowStage.particles = snowStage.particles.filter(p => {
    const surfaceY = getMountainYAt(p.x);
    return p.y < surfaceY;
  });
}
