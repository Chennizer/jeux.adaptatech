document.addEventListener('DOMContentLoaded', () => {
  ...
  function isPointerStageActive() {
    ...
    const optionsVisible = gameOptionsModal && gameOptionsModal.style.display !== 'none';
    const pickerVisible = tilePickerModal && tilePickerModal.style.display !== 'none';
    return !!(tilesVisible && !optionsVisible && !pickerVisible && !videoPlaying);
  }

  function hasPointerCoordinates() {
    return !!(lastPointerPosition && Number.isFinite(lastPointerPosition.x) && Number.isFinite(lastPointerPosition.y));
  }
  ...
  function updatePointerVisibility({ pointerOpacity, skipPosition } = {}) {
    if (!gazePointer) return;
    const pointerEnabled = !!showGazePointer?.checked && isPointerStageActive();
    const hideCursor = pointerEnabled || videoPlaying;
    document.documentElement.classList.toggle('hide-native-cursor', hideCursor);
    if (!pointerEnabled) {
      setPointerDwell(false);
      if (gpDetails) gpDetails.open = false;
    }
    const hasCoords = hasPointerCoordinates();
    const opct = typeof pointerOpacity === 'number' ? pointerOpacity : pointerOpacityFromControls();
    if (pointerEnabled && hasCoords) {
      gazePointer.style.opacity = opct;
      if (!skipPosition) {
  ...
  function refreshPointerStyles() {
    ...
    updatePointerVisibility({ pointerOpacity: opct });
  }
  ...
  if (gazePointer) {
    const rawHandler = (event) => {
      ...
      updatePointerVisibility({ skipPosition: true });
    };
