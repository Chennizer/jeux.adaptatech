const BACK_COLORS = [
  { y: 0, color: [255, 236, 210] },
  { y: 0.45, color: [255, 207, 186] },
  { y: 1, color: [255, 183, 214] }
];
...
export function createBloomScene(p) {
  const petals = [];
  ...
  function spawnSparkles({ reset = true } = {}) {
    ...
    const pulseBase = Math.max(1, Math.ceil(base * 0.5 * INTENSITY_MULTIPLIER));
  ...
  return {
    id: 'bloom',
    name: 'Fleurs lumineuses',
    description: 'Pétales éclatants et éclairs dorés',
    enter() {
    ...
    draw() {
      ...
      if (glow > 0) {
        const maxGlow = Math.min(255, 80 * INTENSITY_MULTIPLIER);
        p.fill(255, 200, 220, maxGlow * glow);
