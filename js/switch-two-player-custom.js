...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function getCategoryState() {
    try {
      ...
      if (allCategory) {
        const allUrls = flattenCategoryUrls(state, { excludeAllVideos: true });
        allCategory.urls = uniqueOrdered(allUrls);
      }
      return state;
    } catch {
      return null;
    ...
    ordered.forEach((cat, idx) => {
      ...
      chip.addEventListener('click', (event) => {
        ...
        if (allCategory && Array.isArray(allCategory.urls)) {
          allCategory.urls = uniqueOrdered(flattenCategoryUrls(nextState, { excludeAllVideos: true }));
        }
  ...
  async function disableCategoriesToFlatList() {
    ...
    if (!state) return;
    syncCategoriesFromDom();
    const flatUrls = uniqueOrdered(flattenCategoryUrls(state, { excludeAllVideos: true }));
